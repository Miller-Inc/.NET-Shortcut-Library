...
using System.IO;
using CMDLibrary.Convert.Strings;
...
namespace CMDLibrary.UI
{
    public class Files
    {
        ...
        public static string GetFilePathUI() 
        {
            ...
            Console.WriteLine("\nChoose a drive: \n" + List_String.ListString(driveList, " \n", "\n"));
            path.Add(GetDriveUI());
            search = true;
            path.Add(input);
            fullPath = path[0];
            Console.WriteLine("\nChoose file or path: \n" + 
                List_String.ListString(GetDirectory(fullPath, path), " \n", " \n"));
            ...
            while (search)
            {
                try
                {
                    ...
                    else if (IsIn(input, GetDirec(fullPath, path)))
                    {
                        Console.WriteLine("Are you sure? ");
                        if (FullStrBool.GetBool(Console.ReadLine()))
                        {
                            input = input.Replace("\\", "");
                    ...
                    else if (IsIn(input, GetFile(fullPath, path)))
                    {
                        Console.WriteLine("Are you sure? ");
                        if (FullStrBool.GetBool(Console.ReadLine()))
                        {
                    ...
                    {
                        List<string> temp = GetDirec(fullPath, path);
                        foreach (string s in GetFile(fullPath, path))
                        {
                            temp.Add(s);
                        }
                        Console.WriteLine(List_String.ListString(temp, "\n", "\n"));
                    }
                    Console.WriteLine("\nChoose file or path: \n" +
                        List_String.ListString(GetDirectory(fullPath, path), " \n", "\n"));
                    Console.WriteLine("Enter \"back\" go back a directory");
        ...
        public static List<string> GetDirectory(string path, List<string> pathList) 
        {
        ...
        public static List<string> GetFile(string path, List<string> pathList) 
        {
            List<string> files = new List<string>();
            List<string> tempDirec = Directory.GetFiles(path).ToList();
            string sub;
        ...
        public static List<string> GetDirec(string path, List<string> pathList)
        {
